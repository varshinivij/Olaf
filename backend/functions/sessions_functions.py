...
def delete_all_sessions(req: Request) -> Response:
    try:
        ...
        return Response(
            json.dumps({"message": "All sessions deleted for user"}),
            status=200,
            mimetype="application/json",
        )
    except Exception as e:
...
def rename_session(req: Request) -> Response:
    try:
        ...
        if not session_id:
            return Response(
                json.dumps({"error": "'session_id' is required"}), status=400
            )
        if not new_name:
...
def update_session(session_id, session_data) -> Session | None:
    """
...
def add_message_to_session(session_id, message, role="user") -> Session | None:
    """
...
def set_sand_box_id(session_id, sandbox_id) -> Session | None:
    """
    Set the sandbox ID for a session
    @return: session_data
    """
    db.collection("sessions").document(session_id).update({"sandboxId": sandbox_id})
    session_ref = db.collection("sessions").document(session_id)
    session_data = session_ref.get().to_dict()
    return session_data  # type: ignore
...
def get_session_summary(req: Request) -> Response:
    try:
        ...
        if not summary_response or "choices" not in summary_response:
            return Response(
                json.dumps({"error": "Failed to generate summary"}),
                status=500,
                mimetype="application/json",
            )
