...
def get_or_create_session(user_id: str, project_id: str, message: str, session_id: str | None = None) -> Tuple[str, Session]:  # type: ignore
    """
    ...
    """
    if session_id:
        session_data = sessions_functions.add_message_to_session(
            session_id, message, role="user"
        )
    else:
        session_id, session_data = sessions_functions.create_new_session(
...
def append_message_to_session(
    session_id: str, message: str, role: str = "assistant"
) -> None:
